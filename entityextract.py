...
linestring = []	
counter = 1
rules = open("keywords_revised.txt", "r")		#open the given rule-book
borders_end = 0
phrase_counter = 0
phrases_to_sort = [[]]
first = False
...
for line_b in rules:
	rule_tofind = re.search(r'(\w+[.]*|%)\s[+]\s(\w+\S*)', line_b)	#for every line in the rule book read out the dependency relation 
	keyword = rule_tofind.group(1)									#and the keyword
	...
	for head in find_relation:		#find all dependency-subtrees of the found word, which teh is the head of those subtrees
		phrase_list.append(head)
		
		for word in phrase_list:
			
			try:
				find_phrase = re.findall(r'<governor idx="' + word[0] + '">' + word[1] + '</governor>\s*<dependent idx="(\d*)">(.*?)</dependent>.*?<sentence id="' + word[2]+ '">', joined, flags=re.DOTALL)
